...
"""
@author: mohit jain
@contributor: Glen-Erik Cortes (mlflow logging and analysis of models and runs)
"""
...
import numpy as np
import pandas as pd
import source.Train.geneticXGboost as geneticXGboost #genetic algorithm module
...
with mlflow.start_run(experiment_id=experimentID, run_name='XGBoostGA', nested=True):
    for generation in range(numberOfGenerations):
        print("This is number %s generation" % (generation))
        #train the dataset and obtain fitness
        fitnessValue = geneticXGboost.train_population(
    ...
    fitness = geneticXGboost.train_population(population=population, experiment_id=experimentID, generation=generation+1, dMatrixTrain=xgDMatrix, dMatrixtest=xgbDMatrixTest, y_train=y_train, y_test=y_test)
