...
def train_population(population, experiment_id, generation, dMatrixTrain, dMatrixtest, y_train, y_test):
    fScore = []
    for i in range(population.shape[0]):
        runname = "gen_" + str(generation) + "_parent_" + str(i)
        print(runname)
        with mlflow.start_run(experiment_id=experiment_id,run_name=runname, nested=True):
